...
function parseFoodInput(input) {
  ...
  for (const item of foodItems) {
    ...
    const match = item.match(
      /^(\d+(?:\.\d+)?)\s*(cup|cups|gm|g|gram|grams|kg|piece|pieces|pcs|pc|slice|slices|tbsp|tsp|ml)?\s*(?:of\s+)?(.+)$/i
    );
    ...
    if (match) {
      ...
      if (foodKey) {
        ...
        const partialMatch = Object.keys(nutritionDatabase).find(
          (key) =>
...
Return ONLY valid JSON. Do not include code fences or extra text.`;
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        console.log(
          `Attempting to get Gemini response for nutrition calculation (Attempt ${attempt}/${MAX_RETRIES})`
        );
        ...
        try {
          ...
          console.log(
            `Successfully got and parsed nutrition data from Gemini on attempt ${attempt}`
          );
    ...
    if (!parsed) {
      console.error(
        `All ${MAX_RETRIES} attempts failed for nutrition calculation. Last error:`,
        lastError?.message
      );
      ...
      Object.keys(totalNutrition).forEach((key) => {
        if (
          totalNutrition[key] !== undefined &&
          typeof totalNutrition[key] === "number"
        ) {
    ...
    if (parsedItems.length === 0) {
      return res.status(400).json({
        success: false,
        message:
          'Could not recognize any food items. Please enter items like "2 chapati, 1 cup daal, 50 gm panner"',
      });
    }
