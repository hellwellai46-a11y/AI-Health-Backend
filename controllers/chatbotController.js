...
Respond in a friendly, conversational manner. Keep responses concise but informative (2-4 paragraphs max).`;
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        console.log(`Attempting to get Gemini response (Attempt ${attempt}/${MAX_RETRIES})`);
        ...
        console.log(`Successfully got response from Gemini on attempt ${attempt}`);
    ...
    return res.status(500).json({
      success: false,
      error: "I'm sorry, I'm having trouble processing your request. Please try again in a moment.",
