...
const isTrustedChannel = (channelTitle, channelId = null) => {
  ...
  return TRUSTED_MEDICAL_CHANNELS.some(trustedChannel => {
    ...
    return lowerTitle.includes(lowerTrusted) || 
           lowerTrusted.includes(lowerTitle) ||
           // Also check for partial matches (e.g., "Apollo" matches "Apollo Hospitals")
           (lowerTitle.split(' ').some(word => word.length > 3 && lowerTrusted.includes(word))) ||
           (lowerTrusted.split(' ').some(word => word.length > 3 && lowerTitle.includes(word)));
  });
};
...
export const getHealthVideos = async (req, res) => {
  try {
    ...
    if (TRUSTED_CHANNEL_IDS.length > 0) {
      console.log(`Searching for videos in ${TRUSTED_CHANNEL_IDS.slice(0, 5).length} trusted channels...`);
      for (const channelId of TRUSTED_CHANNEL_IDS.slice(0, 5)) { // Limit to 5 channels to save API quota
        try {
          const params = {
            part: 'snippet',
          ...
          console.error(`Error fetching videos from channel ${channelId}:`, error.response?.data?.error || error.message);
          if (error.response?.status === 403) {
            console.error('API quota exceeded or invalid key. Status:', error.response.status);
          }
      ...
      console.log(`Strategy 1 found ${videos.length} videos from trusted channels`);
    }
    ...
    if (videos.length < 6) {
      try {
        ...
        if (response.data && response.data.items) {
          console.log(`General search returned ${response.data.items.length} videos`);
          const filteredVideos = response.data.items
            .filter(item => {
              ...
              if (!isTrusted) {
                console.log(`Filtered out: ${channelTitle} (not in trusted list)`);
              }
              return isTrusted;
            })
          ...
          console.log(`Strategy 2 filtered to ${filteredVideos.length} trusted videos`);
          videos.push(...filteredVideos);
        } else {
          console.log('General search returned no results');
        }
      } catch (error) {
        console.error('Error in general search with filtering:', error.response?.data?.error || error.message);
        if (error.response?.status === 403) {
          console.error('API quota exceeded. Full error:', JSON.stringify(error.response.data, null, 2));
        }
      }
    }
    ...
    if (finalVideos.length === 0) {
      ...
      return res.status(200).json({
        success: false,
        data: [],
        error: 'No trusted medical videos found for the given symptoms. This could be due to: API configuration issues, no matching videos in trusted sources, or API quota limits. Please check your YouTube API key configuration.',
        query: symptomsText
      });
    }

    console.log(`Returning ${finalVideos.length} videos for symptoms: ${symptomsText}`);
    ...
    if (error.response) {
      ...
      if (status === 403) {
        ...
        return res.status(200).json({
          success: false,
          data: [],
          error: 'Invalid request to YouTube API. Please check your API key configuration and request parameters.'
        });
      } else if (status === 401) {
        ...
        return res.status(200).json({
          success: false,
          data: [],
          error: 'YouTube API key is invalid or unauthorized. Please check your YOUTUBE_API_KEY in the .env file.'
        });
      }
      ...
      return res.status(200).json({
        success: false,
        data: [],
        error: errorData.error?.message || `Error fetching videos from YouTube API (Status: ${status})`
      });
    }
    ...
    return res.status(200).json({
      success: false,
      data: [],
      error: 'Failed to fetch YouTube videos. Please check your internet connection and API configuration.'
    });
  }
};
